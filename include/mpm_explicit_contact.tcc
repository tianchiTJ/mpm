...
mpm::MPMExplicitContact<Tdim>::MPMExplicitContact(std::unique_ptr<IO>&& io)
    : mpm::MPMBase<Tdim>(std::move(io)) {
  //! Logger
  console_ = spdlog::get("MPMExplicitContact");
}

//! MPM Explicit Contact solver
template <unsigned Tdim>
bool mpm::MPMExplicitContact<Tdim>::solve() {
  bool status = true;

  // Get analysis type USL/USF
  if (io_->analysis_type() == "MPMExplicitContactUSL2D" ||
      io_->analysis_type() == "MPMExplicitContactUSL3D")
    this->usl_ = true;
  ...
  for (; step_ < nsteps_; ++step_) {
    ...
    // Compute nodal velocity of each subdomain
    for (const auto& sub : subdomains) {
    ...
    if (velocity_update_)
      ...
      mesh_->iterate_over_particles(std::bind(
          &mpm::ParticleBase<Tdim>::compute_updated_position_velocity_subdomain,
          std::placeholders::_1, phase, this->dt_));
    else
      // Iterate over each particle to compute updated position
      mesh_->iterate_over_particles(std::bind(
